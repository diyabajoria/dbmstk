...
async function dashboardSummary(req, res, next) {
  try {
    ...
    for (const item of items) {
      ...
      for (const b of item.batches) {
        ...
        if (badge.tier === "EXPIRED") {
          ...
          if (
            badge.tier === "EXPIRES_TODAY" ||
            badge.tier === "EXPIRES_3_DAYS"
          ) {
            needsAttention.push({
              type: "EXPIRING_SOON",
              severity: "HIGH",
      ...
      if (low) {
        ...
        lowStockItems.push({
          ...
          unit: item.unit,
          suggestedRestock: rec
            ? rec.recommendedPurchase
            : Math.max(item.minimumStock - currentStock, 0),
        });
    ...
    // Recent activity — merged purchase + consumption stream
    const [recentPurchases, recentConsumption] = await Promise.all([
      Transaction.find({
        householdId,
        type: "PURCHASE",
      })
        .populate("itemId supplierId")
        .sort({ date: -1 })
        .limit(10),
    ...
    const recentActivity = [
      ...recentPurchases.map((p) => ({
        ...
        date: p.date,
        description:
          `Purchased ${p.quantity} ${p.itemId?.unit || ""} ${
            p.itemId?.name || "item"
          } for ₹${(p.totalAmount || 0).toFixed(0)}`.replace(
            /\s+/g,
            " "
          ),
      })),
      ...
      ...recentConsumption.map((c) => ({
        ...
        date: c.date,
        description:
          `Consumed ${c.quantity} ${c.itemId?.unit || ""} ${
            c.itemId?.name || "item"
          }`.replace(/\s+/g, " "),
      })),
    ]
    ...
    res.json({
      ...
      totalInventoryValue:
        Math.round(totalInventoryValue * 100) / 100,
      ...
      monthlySpending:
        Math.round(monthlySpending * 100) / 100,
